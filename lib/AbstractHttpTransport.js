...
ServiceProvider.set('http', {
	useResourceRoutes(server, res) {

		let params = {
			server,
			resource: res,
			query: res.resource.getTemporalQueryParams()
		};
...
function validateQueryParams(opt) {
	let isValidQuery = opt.query.createQueryValidationFunction(opt.resource);
...
function validateResponseObject(opt) {
	let isValidObject = opt.query.createResponseValidationFunction(opt.resource);
	let getQueryParams = opt.server.getQueryParams;
