...
function resourceObjectRoutes(opt) {
	...
	let find = function (path, resource) {
		let find = resource.find();
		//let ctx = {router: opt.router, state: opt.state};
		opt.server.GET({path, version}, [
			before,
			function (req, res, next) {
				find(opt.state, req.params, req.query, function (err, data) {
					req.result = data;
					console.log(data);
					next();
				});
	...
	for(let entry of opt.state.links.resourceRoutes.entries()) {
		let resource = entry[0];
		let ref = entry[1];
		let path = ref.collection();
...
function relationshipsRoutes(opt) {
	...
	for(let entry of opt.state.links.relationshipRoutes.entries()) {
		let resource = entry[0];
