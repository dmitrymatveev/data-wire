...
class ToOne extends Relationship {
	...
	toJsonApi(ctx, destination, data) {
		...
		// TODO: create an error instead
		if (Array.isArray(value)) {
			value = value[0];
		}

		let related = this.getRelatedResource();
		let linksState = ctx.links;

		if(value === undefined && linksState.useRelationLinks) {
			let ref = getRelationshipsMember(destination, key);
			createLinksObject(linksState, ref.links = ref.links || {}, data.id, related);
		}
		else if (value) {

			let id = value.data || (Array.isArray(value) ? value[0] : value);
			let meta = value.meta;
			let ref = getRelationshipsMember(destination, key);

			if (id) {
				ref.data = {type: related.name, id}
			}

			if (meta) {
				ref.links = ref.links || {};
				if (linksState.useRelationLinks) {
					createLinksObject(linksState, ref.links, data.id, related, meta);
				}
				else {
					ref.links.related = {meta};
				}
			}
		}
...
class ToMany extends Relationship {
	...
	toJsonApi(ctx, destination, data) {
		...
		if (isNullOrUndefined(value) && linksState.useRelationLinks) {
			let ref = getRelationshipsMember(destination, key);
			createLinksObject(linksState, ref.links = ref.links || {}, data.id, related);
		}
		else if (value) {

			let list = value.data || (Array.isArray(value) ? value : null);
			let meta = value.meta;
			let ref = getRelationshipsMember(destination, key);
...
function createLinksObject(linksState, dest, id, related, meta) {
	if (linksState.useSelfLinks) {
		dest.self = linksState.linkToRelationship(id, related);
	}

	let href = linksState.linkToRelatedObject(id, related);
	if (meta) {
		dest.related = {href, meta};
	}
	else {
		dest.related = href;
	}
