...
class RouterService {
	...
	static buildResourceRoutes(router) {
		...
		for(let entry of pm.resources.values()) {
			entry.links = ResourceLinks.createMerged(entry.resource.links, router.links);
			buildRoutes(router, entry);
		}
...
function buildResourceObjectRoutes(router, entry) {
	...
	let path = `${router.urlPrefix}${entry.members.strings.toMany}`;
...
function buildRelationshipsRoutes(router, entry, attribute) {
	...
	let path = `${router.urlPrefix}${entry.members.strings.toMany}/:${entry.resource.urlID}`;
	...
	let relatedResource = pm.resources.get(Resource.toResourceID(attribute.key));
