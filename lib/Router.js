...
class RouterService {
	...
	static buildResourceRoutes(router) {
		let LinksService = ServiceProvider.get('links');
		...
		for(let entry of pm.resources.entries()) {
			...
			LinksService.initialize(state.links);
		}

		for(let entry of pm.resources.entries()) {
			state = entry[1];
			LinksService.build(state.links);
		}
	...
	static getRouterResourceState(router, resource) {
		return PRIVATE.get(router).resources.get(resource);
	}
}
