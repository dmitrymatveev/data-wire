...
class Resource extends AbstractResourceController {
	...
	getTemporalQueryParams() {
		let pm = PRIVATE.get(this);
		if (!pm._builtQueryParams) {
			pm._builtQueryParams = QueryParams.createMerged(
				Resource.getControllerForResource(this).queryParams,
				Resource.getGlobalController().queryParams
			)
		}
		return pm._builtQueryParams;
	}
