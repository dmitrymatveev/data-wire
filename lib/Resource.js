...
function toJsonApi (resource, ctx, obj, callback) {
	...
	err = parseIncludedResources(resource, ctx, result, obj);
	if (err) return callback(err);
...
function parseDataObject(ctx, resource, data) {
	let result = {};
	for(let next of objects.Iterator(PRIVATE.get(resource).schema)) {
		next.toJsonApi(ctx, result, data);
	}
	return result;
}
...
function parseIncludedResources(resource, ctx, destination, obj) {
	...
	if (Array.isArray(obj.included)) {
		...
		for(let item of obj.included) {
			...
			let next = parseDataObject(ctx, related, item);
			if (next === false) {
