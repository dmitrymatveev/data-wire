 ...
 */
ModelInstance.prototype.setProperties = function (properties, onEach) {
    var dataType, k, keys = this._model.definitionKeys;
    for(var i = 0, len = keys.length; i < len; i++) {
        k = keys[i];
        dataType = this._model.definition[k];
        if (!dataType.virtual && properties[k] !== undefined) {
            this[k] = onEach ? onEach(properties[k]) : properties[k];
        }
    }
};
...
ModelInstance.prototype.serialized = function (filter) {
    ...
    for(var i = 0, len = keys.length; i < len; i++) {
	    ...
	    if (!dataType.serialized) {
		    result[k] = this[k];
	    }
	    else {
		    var serialized = dataType.serialized(this[k], k, result);
		    if (serialized) {
			    result[k] = serialized;
		    }
	    }
