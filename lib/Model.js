...
function Model (def, params) {
    ...
    this.actions = {
        ...
        onInstanceRevert : params.onInstanceInit || function () {}
    };
...
ModelInstance.prototype.isDirty = function (propertyName) {
    ...
    else if (def.virtual) throw new VirtualProperty(name);

    return !def.isEqual(this._data[propertyName], this._state.dirty[propertyName]);
};

/**
 * @return {Object} Returns object literal containing all non-virtual properties of this model.
 */
ModelInstance.prototype.serialized = function () {
...
/**
 * @param {Boolean} [dirty] Optianly flag to return only dirty keys.
 * @return {String[]} Returns an array of non-virtual properties of the underlying model.
 */
ModelInstance.prototype.keys = function (dirty) {
    ...
    for(var i = 0, len = keys.length; i < len; i++) {
        k = keys[i];
        dataType = this._model.definition[k];
        if (!dataType.virtual && (!dirty || this.isDirty(k) )) result.push(k);
    }
    return result;
};
    ...
    return this._model.transport[this._state.onSave](this, meta || null).then(function (data) {
        ...
        if ( data ) {
            ...
            for(i = 0; i < keys.length; i++) {
                key = keys[i];
                self._state.dirty[key] = data[key];
            }
        }

        //self.setAll(self._state.dirty, false);

        var dataType, len, value;
        keys = self._model.definitionKeys;

        for(i = 0, len = keys.length; i < len; i++) {
            key = keys[i];
            dataType = self._model.definition[key];
            if (!dataType.virtual) {
                dataType.copyAssignValue(self, key, self._state.dirty[key]);
            }
        }
...
function VirtualProperty(name) {
    this.message = 'Operation not permitted on virtual property "' + name + '".'
}
VirtualProperty.prototype = Object.create(Error.prototype);