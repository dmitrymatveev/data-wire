 ...
 *
 * @param {Function} [params.serializedRoot] Called when models are serialized and is intended to
 * modify root object literal for all serialized instances of this model.
 *
 *  @example
 *  var Jedi = new Model({
...
function Model (def, params) {
    ...
    this.setTransport(this.options.transport || Transport.TransportInterface);
    this.setObjectPool(this.options.objectPool || ObjectPool);

    this.actions = {
        onInstanceInit : this.options.onInstanceInit || function () {},
        onInstanceRevert : this.options.onInstanceRevert || function () {}
    };
...
/**
 * @param {Object} [params] - Same as in Model constructor. If specified, parameters used to instantiate
 * original Model definition will be swapped with this new options.
 * @return {Model} New instance of this Model definition.
 */
Model.prototype.clone = function (params) {
    var options = this.options;
    if (params) {
        options = util._extend({}, options);
        util._extend(options, params);
    }

    return new Model(this.definition, options);
};

/**
 * Returns a copy of this model extended with the provided definition and options.
 * @param {Object} def
 * @param {Object} params
 * @returns {Model}
 */
Model.prototype.extend = function (def, params) {
	var definition = util._extend(util._extend({}, this.definition), def);
	var options = util._extend(util._extend({}, this.options), params);
	return new Model(definition, options);
};
...
Model.prototype.findAll = function (keys, meta) {
	var model = this;
    return this.transport.readAll(keys, meta || null, model).then(function (dataArr) {

	    var res = [];

	    for(var i = 0, len = dataArr ? dataArr.length : 0; i < len; i++) {
		    var next = dataArr[i];
		    ...
		    var instance = model.objectPool.acquire();
		    instance.init(TRANSPORT_ACTION.DO_NOTHING, next, false);
		    res.push(instance);
	    }
 ...
 * @param value
 */
ModelInstance.prototype.setProperty = function (key, value) {
	if (this._model.definitionKeys.indexOf(key) >= 0) {
		this[key] = value;
	}
};
 ...
 */
ModelInstance.prototype.setProperties = function (properties) {
    var dataType, k, keys = this._model.definitionKeys;
    for(var i = 0, len = keys.length; i < len; i++) {
        k = keys[i];
        dataType = this._model.definition[k];
 ...
 * @returns {{}}
 */
ModelInstance.prototype.toJSON = function (filter, callback) {

	var root = {};
	var result = null;
	if (this._model.options.serializedRoot) {
		result = this._model.options.serializedRoot(root);
	}
	else {
		result = root;
	}

	var dataType, k, keys = filter ? filter : this.propertyFilter(DEFAULT_PROP_FILTER);
	for(var i = 0, len = keys.length; i < len; i++) {
		k = keys[i];
		dataType = this._model.definition[k];
		...
		if (!callback) {
			...
			var out = callback(dataType, k, this[k], result, root);
			if (out !== undefined) {
				result[k] = out;
			}
		}
...
ModelInstance.prototype.serialized = function (filter) {
	...
	return this.toJSON(filter, function (type, key, value, data, root) {
		return type.serialized ? type.serialized(value, key, data, root) : value;
	});
};
...
ModelInstance.prototype.keys = function (dirty, type) {
    ...
    //console.warn('DEPRECATION: ModelInstance.keys method is deprecated, please use ModelInstance.propertyFilter instead!');
    //console.trace();
 ...
 * @property {Boolean} [virtual=false]
 * @property {DataType} [type]
 * @property {Boolean} [dirty]
 * @property {Function} [validate]
 */
const DEFAULT_PROP_FILTER = {
	all: null,
    virtual: false,
    type: DataType.Any,
    dirty: null,
...
/**
 * @param {PropertyFilterOptions} [filter] If not filter is given defaults will be used.
 *
 * Default property filter object:
 * 		- all: not set
 * 		- virtual: false
 * 		- type: Any
 * 		- dirty: not set
 ...
 * @return {String[]}
 */
ModelInstance.prototype.propertyFilter = function (filter) {
	...
	if (!!opt.all && opt.all) {
		return this._model.definitionKeys.concat([]);
	}

    var dataType, k, keys = this._model.definitionKeys;
    var virtual = opt.virtual === undefined ? DEFAULT_PROP_FILTER.virtual : opt.virtual;

    for(var i = 0, len = keys.length; i < len; i++) {
        k = keys[i];
        dataType = this._model.definition[k];

        if (dataType.virtual !== virtual) continue;
        if (opt.type && !opt.type.testType(dataType)) continue;
        if (!!opt.dirty && this.isDirty(k) !== opt.dirty) continue;
        if (!!opt.validate && !opt.validate(this[k])) continue;
        ...
        for(var i = 0, len = keys.length; i < len; i++) {
            ...
            if (!dataType.virtual) {
                ...
                if (data && data[key]) {
					...
					if (dataType.valueDecorator) {
						val = dataType.valueDecorator(val, self, key);
					}
                    dataType.copyAssignValue(self, key, val);
                }
                else if (self._state.dirty[key] !== undefined) {
